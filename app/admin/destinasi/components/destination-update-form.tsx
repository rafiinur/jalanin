...
import { updateDestination } from "@/libs/destinations";
...
const DestinationEditForm = ({
	...
	const { mutateAsync: updateDestinationMutate } = useMutation({
		mutationFn: ({
			...
			formData: FormData;
		}) => updateDestination(destinationId, formData),
		onSuccess: () => {
	...
	const onSubmit = async (data: z.infer<typeof destinationSchema>) => {
		try {
			...
			if (data.image_url instanceof File) {
				...
				typeof destination.image_url === "string" &&
